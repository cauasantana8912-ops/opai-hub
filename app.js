...
function showApp(){const u=currentUser();if(!u)return showAuth();$('authView').classList.add('hidden');$('appView').classList.remove('hidden');$('desktopUser').textContent='@'+u.username;$('userCount').textContent=getUsers().length;$('desktopAdmin').classList.toggle('hidden',!u.admin);renderCards('desktopCards');renderCards('mobileCards');renderOffline();applyTheme()}
...
$('desktopAdmin').onclick=()=>{if(currentUser()?.admin)$('adminDialog').showModal()};$('closeAdmin').onclick=()=>$('adminDialog').close();
...
applyTheme();if(localStorage.getItem(SESSION_KEY))showApp();else showAuth();
